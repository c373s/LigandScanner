...
### Minimum fragment number indicate filter for number of reporter ions, can be adapted accordingly
FragTol<-0.01
FragTol<-FragTol^2
Min_mass_shift<-40
Min_frag_number<-8
...
MR1_data<-readMgfData(filename = "YourFile.mgf", 
...
write.csv(IonCheck_MR1_data_filtered, file = "LigandScannerBinary.csv", quote = FALSE, row.names = FALSE, sep = ";")
write.csv(IonCheck_MR1_data_filtered_absolute, file = "LigandScannerAbolute.csv", quote = FALSE, row.names = FALSE, sep = ";")
########################################################################################
